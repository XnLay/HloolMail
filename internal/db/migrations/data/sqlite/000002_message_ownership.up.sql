...
WHERE owner_id IS NULL AND deleted_at IS NULL
  ...
  AND EXISTS (
    ...
    WHERE mailboxes.id = messages.mailbox_id
      ...
      AND julianday(mailboxes.created_at) <= julianday(mailboxes.updated_at)
      AND julianday(mailboxes.updated_at) < julianday(messages.created_at)
  );
...
WHERE owner_id IS NULL AND deleted_at IS NULL
  ...
  AND EXISTS (
    ...
    WHERE domains.id = messages.domain_id
      ...
      AND julianday(domains.created_at) <= julianday(domains.updated_at)
      AND julianday(domains.updated_at) < julianday(messages.created_at)
  );
